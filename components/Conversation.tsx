...
console.log('Using AGENT_ID:', ELEVEN_AGENT_ID);

const RETELL_API_KEY = process.env.RETELL_API_KEY || '';
console.log('Using RETELL_API_KEY:', RETELL_API_KEY);
    ...
    const handle11Start = async () => {
        ...
        try {
            const response = await fetch('/api/get-elevenlabs-signed-url', {
                ...
                body: JSON.stringify({ ELEVEN_AGENT_ID })
            });
            ...
            const {
                ...
                status,
                isSpeaking,
            } = elevenLabsModule.useConversation({
    ...
    return (
        <div className="flex flex-col items-center">
            ...
            {startInterview == 'elevenlabs' && (
                <div className="text-black mt-2"> ElevenLabs Status: {status}, Speaking: {elevenLabsModule.isSpeaking ? 'Yes' : 'No'} </div>
            )}
        </div>
    );
});
